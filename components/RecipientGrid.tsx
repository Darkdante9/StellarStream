...
const RecipientRow = React.memo(function RecipientRow({
  ...
  const getInputProps = (field: keyof Recipient) => {
    ...
    const baseProps: React.InputHTMLAttributes<HTMLInputElement> = {
      ...
      className: `w-full px-2 py-1 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 ${validationErrors?.[field] ? 'border-red-300 focus:ring-red-500' : 'border-gray-300'
        }`,
...
export const RecipientGrid = React.memo(function RecipientGrid({
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
      ...
      {recipients.length === 0 && (
        <div className="text-center py-8 text-gray-500 animate-fade-in" role="status">
          No recipients added yet. Add recipients to get started.
        </div>
      )}

      {hasSelection && (
        <div className="px-4 py-2 bg-gray-50 border-t border-gray-200 text-sm text-gray-600 animate-fade-in">
